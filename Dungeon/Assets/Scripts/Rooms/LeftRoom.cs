...
public class LeftRoom : Room {

    public LeftRoom(Room ParentRoom, int RoomIndex, int Width, int Height, Vector3Int Location){
        ...
        this.location = new Vector3Int(Location.x - (width-1), Location.y - Random.Range(1,height-2), 0);
        ...
        if(!isDeleted){
        // Debug.Log("LeftRoom: " + this.roomIndex + ", parent room: " + this.parentRoom.roomIndex);
            this.doorTop = new Vector3Int(location.x + Random.Range(1,width-1), location.y + (height-1), 0);
            this.doorRight = Location;
            ...
            if(availableDoor1 == 1 || availableDoor2 == 1) BottomRoom.GenerateBottomRoom(this, DungeonGenerator.rooms, new Vector3Int(doorBottom.x, doorBottom.y - 2, 0));
            if(availableDoor1 == 2 || availableDoor2 == 2) GenerateLeftRoom(this, DungeonGenerator.rooms, new Vector3Int(doorLeft.x - 2, doorLeft.y, 0));
            for(int i = 0; i < width + 2; i++){
                for(int j = 0; j < height + 2; j++){
    ...
    public void DrawLeftRoom(){
        ...
        // DungeonGenerator.dungeon.SetTile(new Vector3Int(doorRight.x,doorRight.y,0), DungeonGenerator.tiles[this.color]);
        // DungeonGenerator.dungeon.SetTile(new Vector3Int(doorRight.x+1,doorRight.y,0), DungeonGenerator.tiles[this.color]);
        // DungeonGenerator.dungeon.SetTile(new Vector3Int(doorRight.x+2,doorRight.y,0), DungeonGenerator.tiles[this.color]);
        
        // foreach(Room room in DungeonGenerator.RoomsSorted){
        //     if(room.roomIndex == this.parentRoomIndex){
                if(this.parentRoom.isCleared){
                    for(int i = -1; i < 2; i++){
                        DungeonGenerator.dungeon.SetTile(new Vector3Int(doorRight.x,doorRight.y + i,0), DungeonGenerator.tiles[Random.Range(1,9)]);
                        DungeonGenerator.dungeon.SetTile(new Vector3Int(doorRight.x+1,doorRight.y + i,0), DungeonGenerator.tiles[Random.Range(1,9)]);
                        DungeonGenerator.dungeon.SetTile(new Vector3Int(doorRight.x+2,doorRight.y + i,0), DungeonGenerator.tiles[Random.Range(1,9)]);
                    }
                    // foreach(Room room in DungeonGenerator.RoomsSorted){
    ...
    public static void GenerateLeftRoom(Room ParentRoom, int RoomIndex, Vector3Int Location){
        int Width = Random.Range(10,18);
        int Height = Random.Range(10,14); 
        // LeftRoom leftRoom = new LeftRoom(RoomIndex+1, Width, Height, new Vector3Int(Location.x, Location.y, 0));
        DungeonGenerator.Rooms.Add(new LeftRoom(ParentRoom, RoomIndex, Width, Height, new Vector3Int(Location.x, Location.y, 0)));
    }
}
